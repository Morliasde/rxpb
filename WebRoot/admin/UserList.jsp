					...
					<c:forEach var="user" items="${users}" varStatus="order">
						<tr <c:if test="${order.count%2==0 }">bgcolor="cacaca"</c:if>>
							...
							<td><a href="admin/getUserDetail?userId=${user.userId}">编辑</a>
								<a href="admin/disableUser?userId=${user.userId}">禁用</a>
								<a href="admin/changeUserCompatitionId?userId=${user.userId}">
								<c:if test="${user.competitionId==0 }">仅当前赛事有效</c:if>
								<c:if test="${user.competitionId>0 }">所有赛事有效</c:if>
								</a>
							</td>
						</tr>
					</c:forEach>
